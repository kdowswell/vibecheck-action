...
export const aiDetectorParams = z.object({
  diff: z.string().describe('Git diff to analyze for AI-generated patterns'),
  commitCount: z.number().optional().describe('Number of commits in PR'),
});

export type AIDetectorParams = z.infer<typeof aiDetectorParams>;

export async function aiDetectorHandler(params: AIDetectorParams): Promise<string> {
  const { diff, commitCount = 1 } = params;
  const signals: string[] = [];
  const sections: AIDetectionResult['sections'] = [];
  ...
  let confidence = 0;
  if (additionRatio > 5 && totalAdditions > 100) confidence += 0.25;
  if (commitCount === 1 && totalAdditions > 200) confidence += 0.2;
  if (verboseComments > 5) confidence += 0.15;
  if (stepComments > 2) confidence += 0.15;
  if (genericNames > 5) confidence += 0.1;
  if (todoPlaceholders > 2) confidence += 0.1;
...
function analyzeBlock(
  ...
  filename: string, 
  startLine: number
): AIDetectionResult['sections'][0] | null {
